...
func load(model_spec: Dictionary) -> Dictionary:
	...
	_debug_state["failure_message"] = "ONNX depth execution is not wired in this repo yet. Artifact resolution works, but no runnable ONNX inference substrate exists here today."
	_debug_state["active_model_summary"] = "enabled; artifact resolved to %s via onnx but execution is not wired yet" % String(_debug_state.get("family_id", "unknown"))
	return {
		"ok": false,
