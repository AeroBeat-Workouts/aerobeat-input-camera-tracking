...
const MAX_BUFFER_SIZE := 524288  # 512KB cap so a few 1080p JPEGs can queue before trimming
const BUFFER_TRIM_TARGET_SIZE := 262144  # Drop stale data back to a smaller working tail on overflow
...
var MJPEG_HEADER_END_BYTES := PackedByteArray([0x0D, 0x0A, 0x0D, 0x0A])
var MJPEG_BOUNDARY_BYTES := PackedByteArray([0x2D, 0x2D, 0x66, 0x72, 0x61, 0x6D, 0x65, 0x2D, 0x62, 0x6F, 0x75, 0x6E, 0x64, 0x61, 0x72, 0x79])  # "--frame-boundary"
var MJPEG_CONTENT_LENGTH_HEADER_BYTES := PackedByteArray([0x43, 0x6F, 0x6E, 0x74, 0x65, 0x6E, 0x74, 0x2D, 0x4C, 0x65, 0x6E, 0x67, 0x74, 0x68, 0x3A, 0x20])  # "Content-Length: "
var CRLF_BYTES := PackedByteArray([0x0D, 0x0A])
var LF_LF_BYTES := PackedByteArray([0x0A, 0x0A])
	...
	while _thread_running and _tcp:
		...
		if available > 0:
			...
			if chunk[0] == OK:
				...
				# Buffer overflow protection - drop stale data but keep the newest parseable tail
				if _mjpeg_buffer.size() > MAX_BUFFER_SIZE:
					header_parsed = _trim_mjpeg_buffer_on_overflow(header_parsed)

				# Parse HTTP headers first (search for \r\n\r\n as bytes, not UTF-8)
				if not header_parsed:
					var header_end := _find_byte_pattern(_mjpeg_buffer, MJPEG_HEADER_END_BYTES)
					if header_end != -1:
...
func _find_last_byte_pattern(buffer: PackedByteArray, pattern: PackedByteArray) -> int:
	"""Find the last occurrence of a byte pattern in buffer. Returns -1 if not found."""
	if pattern.is_empty() or buffer.size() < pattern.size():
		return -1

	for i in range(buffer.size() - pattern.size(), -1, -1):
		var found := true
		for j in range(pattern.size()):
			if buffer[i + j] != pattern[j]:
				found = false
...
func _trim_mjpeg_buffer_on_overflow(header_parsed: bool) -> bool:
	_mjpeg_overflow_count += 1
	var original_size: int = _mjpeg_buffer.size()
	var keep_from: int = maxi(original_size - BUFFER_TRIM_TARGET_SIZE, 0)

	if header_parsed:
		var boundary_in_tail: int = _find_byte_pattern(_mjpeg_buffer.slice(keep_from), MJPEG_BOUNDARY_BYTES)
		if boundary_in_tail != -1:
			keep_from += boundary_in_tail
		else:
			var last_boundary: int = _find_last_byte_pattern(_mjpeg_buffer, MJPEG_BOUNDARY_BYTES)
			if last_boundary != -1:
				keep_from = last_boundary
...
func _parse_mjpeg_frame() -> bool:
	...
	var next_boundary := _find_byte_pattern(_mjpeg_buffer.slice(boundary_pos + MJPEG_BOUNDARY_BYTES.size()), MJPEG_BOUNDARY_BYTES)
	if next_boundary == -1:
		return false
	next_boundary += boundary_pos + MJPEG_BOUNDARY_BYTES.size()  # Adjust offset back to absolute
	...
	var content_length_start := _find_byte_pattern(frame_data, MJPEG_CONTENT_LENGTH_HEADER_BYTES)
	var jpeg_data: PackedByteArray

	if content_length_start != -1:
		# Find end of line (\r\n) after Content-Length
		var line_end := _find_byte_pattern(frame_data.slice(content_length_start + MJPEG_CONTENT_LENGTH_HEADER_BYTES.size()), CRLF_BYTES)
		if line_end != -1:
			# Extract just the number as bytes, then parse it
			var num_start := content_length_start + MJPEG_CONTENT_LENGTH_HEADER_BYTES.size()
			...
			# Find header end (\r\n\r\n) in frame data
			var header_end := _find_byte_pattern(frame_data, MJPEG_HEADER_END_BYTES)
			if header_end == -1:
				header_end = _find_byte_pattern(frame_data, LF_LF_BYTES)  # Fall back to \n\n
			if header_end != -1:
