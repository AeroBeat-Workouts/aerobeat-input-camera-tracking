...
func get_tracking_session():
	if _provider != null and _provider_lane == PROVIDER_LANE_CAMERA_TRACKING:
		var provider_tracking_session: Variant = _provider.get("_tracking_session")
		if provider_tracking_session is Node:
			return provider_tracking_session
...
func get_body_grid_nose() -> Dictionary:
	_ensure_provider()
	if _provider == null or not _provider.has_method("get_body_grid_nose"):
		return {}
	return _provider.get_body_grid_nose()

func get_body_grid_left_wrist() -> Dictionary:
	_ensure_provider()
	if _provider == null or not _provider.has_method("get_body_grid_left_wrist"):
		return {}
	return _provider.get_body_grid_left_wrist()

func get_body_grid_right_wrist() -> Dictionary:
	_ensure_provider()
	if _provider == null or not _provider.has_method("get_body_grid_right_wrist"):
		return {}
	return _provider.get_body_grid_right_wrist()

func get_body_grid_calibration_state() -> Dictionary:
	_ensure_provider()
	if _provider == null or not _provider.has_method("get_body_grid_calibration_state"):
		return {}
	return _provider.get_body_grid_calibration_state()
...
func _ensure_camera_tracking_provider(tracking_session, manage_tracking_session_lifecycle: bool = false) -> void:
	...
	if _provider != null and _provider_lane == PROVIDER_LANE_CAMERA_TRACKING:
		...
		_provider.manage_tracking_session_lifecycle = manage_tracking_session_lifecycle
	...
	provider.manage_tracking_session_lifecycle = manage_tracking_session_lifecycle
...
func _discover_tracking_session():
	if not is_inside_tree():
		return null
	var direct_tracking_session := get_node_or_null(TRACKING_SESSION_NODE_NAME)
	if direct_tracking_session != null:
		return direct_tracking_session
	var child_tracking_session := find_child(TRACKING_SESSION_NODE_NAME, true, false)
	if child_tracking_session != null:
		return child_tracking_session
