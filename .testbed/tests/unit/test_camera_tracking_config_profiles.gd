...
func test_camera_tracking_config_loads_boxing_profile_bundle_from_canonical_paths() -> void:
	...
	assert_eq(int(bundle.get("gesture_detection", {}).get("hook", {}).get("evaluation", {}).get("window_ms", -1)), 250)
	assert_false(bundle.get("gesture_detection", {}).get("hook", {}).get("evaluation", {}).has("wrist_velocity_window_ms"))
	...
	assert_false(bundle.get("gesture_detection", {}).get("straight_punch", {}).get("thresholds", {}).has("min_forward_depth_spike"))
	...
	assert_eq(String(bundle.get("gesture_detection", {}).get("learned_classifier", {}).get("model", {}).get("artifact_path", "")), "res://addons/aerobeat-input-camera-tracking/docs/baselines/boxing-punch-classifier-frozen-benchmark-mlp-vs-cnn-2026-06-16/mlp/mlp-result.json")
