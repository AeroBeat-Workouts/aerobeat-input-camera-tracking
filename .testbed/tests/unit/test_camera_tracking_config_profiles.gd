...
func test_camera_tracking_config_loads_boxing_profile_bundle_from_canonical_paths() -> void:
	...
	assert_true(is_equal_approx(float(bundle.get("gesture_detection", {}).get("straight_punch", {}).get("threshold", {}).get("thresholds", {}).get("max_wrist_shoulder_xy_distance", -1.0)), 0.18))
	...
	assert_eq(int(bundle.get("gesture_detection", {}).get("straight_punch", {}).get("threshold", {}).get("timing", {}).get("triggered_grace_ms", -1)), 1)
