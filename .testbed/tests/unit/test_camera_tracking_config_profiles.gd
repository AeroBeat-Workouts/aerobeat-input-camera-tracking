...
func test_camera_tracking_config_loads_boxing_profile_bundle_from_canonical_paths() -> void:
	...
	assert_true(is_equal_approx(float(bundle.get("gesture_detection", {}).get("calibration", {}).get("t_pose", {}).get("grid_size_multiplier", -1.0)), 1.1))
	assert_true(is_equal_approx(float(bundle.get("gesture_detection", {}).get("calibration", {}).get("t_pose", {}).get("camera_space_grid_height_offset", 99.0)), -0.05))
	...
	assert_false(bool(bundle.get("gesture_detection", {}).get("hook", {}).get("grid_detection", {}).get("evaluation", {}).get("overflow_protection_enabled", true)))
	...
	assert_false(bool(bundle.get("gesture_detection", {}).get("uppercut", {}).get("grid_detection", {}).get("evaluation", {}).get("overflow_protection_enabled", true)))
	...
	assert_eq(int(bundle.get("gesture_detection", {}).get("straight_punch", {}).get("threshold", {}).get("timing", {}).get("triggered_grace_ms", -1)), 240)
	...
	assert_eq(int(bundle.get("gesture_detection", {}).get("straight_punch", {}).get("threshold", {}).get("rearm", {}).get("pose_only_rearm_ms", -1)), 250)
