...
func test_camera_tracking_config_loads_boxing_profile_bundle_from_canonical_paths() -> void:
	...
	assert_true(bool(bundle.get("gesture_detection", {}).get("straight_punch", {}).get("threshold", {}).get("depth", {}).get("enabled", false)))
	assert_true(bool(bundle.get("gesture_detection", {}).get("hook", {}).get("threshold", {}).get("depth", {}).get("enabled", false)))
	assert_true(bool(bundle.get("gesture_detection", {}).get("uppercut", {}).get("threshold", {}).get("depth", {}).get("enabled", false)))
