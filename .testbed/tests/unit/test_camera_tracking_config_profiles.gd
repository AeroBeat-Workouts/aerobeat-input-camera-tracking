...
func test_camera_tracking_config_loads_boxing_profile_bundle_from_canonical_paths() -> void:
	...
	assert_eq(int(bundle.get("camera_tracking", {}).get("preview", {}).get("live", {}).get("max_fps", -1)), 10)
	...
	assert_eq(int(bundle.get("camera_tracking", {}).get("source", {}).get("live_camera", {}).get("requested_fps", -1)), 15)
