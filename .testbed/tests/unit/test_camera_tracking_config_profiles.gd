...
func test_camera_tracking_config_loads_boxing_profile_bundle_from_canonical_paths() -> void:
	...
	assert_eq(int(bundle.get("gesture_detection", {}).get("hook", {}).get("grid_detection", {}).get("evaluation", {}).get("min_column_delta", -1)), 2)
	...
	assert_eq(int(bundle.get("gesture_detection", {}).get("uppercut", {}).get("grid_detection", {}).get("evaluation", {}).get("min_row_delta", -1)), 2)
