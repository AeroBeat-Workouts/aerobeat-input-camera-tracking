...
func test_boxing_event_feed_text_lists_hook_uppercut_and_guard_tuning_sections() -> void:
	...
	assert_string_contains(text_body, "Hook direction reference: athlete-space outward columns (left hook = athlete_left with negative signed delta, right hook = athlete_right with positive signed delta)")
