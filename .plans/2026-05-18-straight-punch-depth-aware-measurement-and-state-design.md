...
**Status:** ⚠️ Partial / blocked on truthful rearm-side-lock behavior

**Results:**
- Task 1 completed and the implementation slice remained the recommended Boxing-only coupled model: mirrored 3D straight-punch metrics, per-side `armed → extending → recovering` state, `armed_forward_distance` / `peak_forward_distance`, and proving-harness debug surfacing for the new measurements/state.
- Fresh coder validation resumed from the preserved WIP with scripted guarded-left fixture captures and preserved new evidence under `.testbed/test-results/runner-boxing/`:
  - `20260519-201201__boxing_punch_left_x4_while_guarding_take_01/` — first relaunch rerun, but it only captured about `2989 ms` of fixture-relative state and never reached authored windows 3-4, so it was preserved as a truncated artifact rather than treated as detector truth.
  - `20260519-201255__boxing_punch_left_x4_while_guarding_take_01/` — first full-length rerun of the preserved coupled-model WIP. Result: **0/4 authored `punch_left` hits**, **0 straight-punch false positives**, but the new state never left `armed`; `boxing_debug.left_straight.own_half_lock` stayed false through all authored punch windows.
  - `20260519-201931__boxing_punch_left_x4_while_guarding_take_01/` — a narrow follow-up experiment that changed only the straight-punch own-half lock to a shoulder-relative lateral-dominance check. Result: **1/4 authored-window hits** (window 4 at `4943 ms`) but **7 total `punch_left` events**, meaning **6 false positives** (`462`, `1329`, `1686`, `2852`, `4088`, `6493 ms`) and clearly untruthful rearm / side-lock behavior.
- Because the own-half-lock experiment immediately created straight-punch spam, the coding pass stopped there per the task guardrail and the widened detector logic was **not kept**. The repo was restored to the preserved WIP detector behavior instead of silently broadening the slice.
- The living HTML log was updated before each real attempt and now records the truncated rerun, the full no-hit rerun, and the rejected spammy lock experiment with exact artifact folders and event counts.
- Targeted repo-local validation for the resumed slice completed on the kept repo state:
  - `PYTHONPATH=scripts python3 -m unittest scripts/test_proving_fixture_runner.py`
  - `godot --headless --path .testbed -s res://addons/gut/gut_cmdln.gd -gdir=res://tests/unit -gselect=test_pose_detector_substrate -gexit`
- Straight-punch unit coverage was tightened while keeping scope narrow: the substrate unit test now explicitly forces the per-side straight-punch mini-state into `armed` and verifies that mirrored depth-aware left/right punch frames still emit `punch_left` / `punch_right` once the state is armed, without depending on unrelated flow-swing heuristics.
